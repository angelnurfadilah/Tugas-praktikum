...
int main()
{
	string nmMahasiswa[100];
        string jlhMahasiswa[100];  
        string kMahasiswa[100];
	...
	while(entry == 'Y' || entry == 'y')
	{
		...
		if(kMahasiswa[i] == "SI"){
			jlhMahasiswa[i] = "Sistem Informasi";
			bayarAwal[i] = 2100000;
			bayarCicilan[i] = 7;
			besaran[i] = 300000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
		}else if(kMahasiswa[i] == "TI"){
			jlhMahasiswa[i] = "Teknik Informatika";
			bayarAwal[i] = 2500000;
			bayarCicilan[i] = 7;
			besaran[i] = 300000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
		}else if(kMahasiswa[i] == "KA"){
			jlhMahasiswa[i] = "Komputer Akuntansi";
			bayarAwal[i] = 1750000;
			bayarCicilan[i] = 6;
			besaran[i] = 200000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
		}else if(kMahasiswa[i] == "MI"){
			jlhMahasiswa[i] = "Manajemen Informatika";
			bayarAwal[i] = 1500000;
			bayarCicilan[i] = 6;
			besaran[i] = 250000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
